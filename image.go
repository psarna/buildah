...
func (i *containerImageRef) NewImageSource(_ context.Context, _ *types.SystemContext) (src types.ImageSource, err error) {
	...
	for _, layerID := range layers {
		...
		if apiLayerIDs[layerID] {
			...
			logrus.Info("diffFileName", diffFilename)
			if err != nil {
		...
		logrus.Info("media type", omediaType, dmediaType)
		...
		if i.confidentialWorkload.Convert {
			...
			if apiLayerIDs[layerID] {
				...
				logrus.Info("storediff", diffOptions)
				...
				logrus.Info("storediff done", layerID, diffOptions)
			}
		}
		...
		// digesters.
		logrus.Info("iocopy start")
		size, err := io.Copy(writer, rc)
		logrus.Info("iocopy end", err)
